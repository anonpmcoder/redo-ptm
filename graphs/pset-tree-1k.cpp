...
#define DATA_FILE "data/pset-tree-1k-cxptm.txt"
...
int main(void) {
    ...
    vector<int> ratioList = { 1000, 100, 10 };                        // Permil ratio: 100%, 10%, 0%
    ...
    for (unsigned ir = 0; ir < ratioList.size(); ir++) {
        ...
        for (unsigned it = 0; it < threadList.size(); it++) {
...
#ifdef USE_CXPTM
            results[ic][it][ir] = bench.benchmark<TMRedBlackTree<uint64_t,uint64_t,cx::CX,cx::persist>,                              cx::CX>                  (cNames[ic], nThreads, ratio, testLength, numRuns, numElements, false);
            ic++;
#elif defined USE_CXPUC
            ...
            ic++;
#elif defined USE_REDO
            results[ic][it][ir] = bench.benchmark<TMRedBlackTree<uint64_t,uint64_t,redo::Redo,redo::persist>, redo::Redo>          (cNames[ic], nThreads, ratio, testLength, numRuns, numElements, false);
            ic++;
#elif defined USE_REDOTIMED
            results[ic][it][ir] = bench.benchmark<TMRedBlackTree<uint64_t,uint64_t,redotimed::RedoTimed,redotimed::persist>, redotimed::RedoTimed>          (cNames[ic], nThreads, ratio, testLength, numRuns, numElements, false);
            ic++;
#elif defined USE_REDOOPT
            results[ic][it][ir] = bench.benchmark<TMRedBlackTree<uint64_t,uint64_t,redoopt::RedoOpt,redoopt::persist>, redoopt::RedoOpt>          (cNames[ic], nThreads, ratio, testLength, numRuns, numElements, false);
            ic++;
#elif defined USE_OFWF
            results[ic][it][ir] = bench.benchmark<TMRedBlackTree<uint64_t,uint64_t,onefileptmwf::OneFileWF,onefileptmwf::tmtype>,    onefileptmwf::OneFileWF> (cNames[ic], nThreads, ratio, testLength, numRuns, numElements, false);
            ic++;
#elif defined USE_PMDK
