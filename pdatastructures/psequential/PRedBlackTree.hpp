#ifndef _PERSISTENT_SEQUENTIAL_RED_BLACK_TREE_H_
#define _PERSISTENT_SEQUENTIAL_RED_BLACK_TREE_H_
...
class PRedBlackTree {
    struct Node {
        ...
        Node*   left {nullptr};
        Node*   right {nullptr};
        int64_t color;    // color of parent link
        int64_t size;     // subtree count
    ...
    inline void assignAndFreeIfNull(Node*& z, Node* w) {
        Node* tofree = z;
        z = w;
...
public:
    ...
    PRedBlackTree(const PRedBlackTree& other) {
        if (other.root != nullptr) {
            root = TM::template tmNew<Node>(*other.root);
            copyAll(root, other.root);
        }
    }

    void copyAll(Node* node, Node* onode) {
        if (onode->left != nullptr) {
            node->left = TM::template tmNew<Node>(*onode->left);
            copyAll(node->left, onode->left);
        }
        if (onode->right != nullptr) {
            node->right = TM::template tmNew<Node>(*onode->right);
            copyAll(node->right, onode->right);
        }
    }
    ...
    void deleteAll(Node* rt) {
        Node* left = rt->left;
        if (left != nullptr) deleteAll(left);
        Node* right = rt->right;
        if (right != nullptr) deleteAll(right);
        TM::tmDelete(rt);
    }
    ...
    */
    ...
    void addAll(K** keys, int size) {
        for (int i = 0; i < size; i++) add(*keys[i]);
    }

    static std::string className() { return "PRedBlackTree"; }

};

#endif   // _PERSISTENT_SEQUENTIAL_RED_BLACK_TREE_H_
