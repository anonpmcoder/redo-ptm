...
#define DEFER_PWB(_addr) if (tlocal.tl_pwb_idx < PWB_LOG_SIZE) { tl_pwb_log[tlocal.tl_pwb_idx++] = (_addr); } else { gCX.flushDeferredPWBs(); PWB(_addr);}
    ...
    inline void copy_redolog(State* state, uint64_t redoSize, int tid, const uint64_t offset) noexcept {
        ...
        while(true){
            ...
            for(int i=0;i<len;i++){
            	...
            	DEFER_PWB(ADDR2CL(e.addr + offset));
...
public:
     ...
     */
    inline bool add_to_log(void* addr, uint64_t oldval, uint64_t val) noexcept {
    	Redo::State* state = (Redo::State*)tlocal.st;
    ...
    template<typename R,class F> inline static R readTx(F&& func) {
        return gCX.ns_read_transaction<R>(std::forward<F>(func));
    }

    template<typename R,class F> inline static R updateTx(F&& func) {
        return gCX.ns_write_transaction<R>(std::forward<F>(func));
    }
...
template<typename T> inline void persist<T>::pstore(T newVal) {
	...
	if (offset != 0 && ADDR_IS_IN_MAIN(valaddr)) {
		uint64_t oldval = *reinterpret_cast<uint64_t*>( valaddr + offset );
		bool sameCL = gCX.add_to_log(valaddr, oldval, (uint64_t)newVal);
		...
		bool sameCL = gCX.add_to_log(valaddr - offset, (uint64_t)val, (uint64_t)newVal);
