...
class RedoOpt {
    ...
    static const int IDX_BITS = 16;  // WARNING: ring capacity up to 4096
    ...
    struct CLAggregate {
		...
		inline void addIfAbsent(void* cl) {
			...
			if(numCL!=0){
		    	...
		    	if(numCL+1 > 3*gCX.esloco.getUsedSize()/(64*4)) {
		    		tlocal.copy = true;
		    		numCL = 0;
		    		return;
		    	}
...
//
template<typename R, typename F> static R updateTx(F&& func) { return gCX.updateTx<R>(func); }
template<typename R, typename F> static R readTx(F&& func) { return gCX.readTx<R>(func); }
template<typename F> static void updateTx(F&& func) { gCX.updateTx(func); }
template<typename F> static void readTx(F&& func) { gCX.readTx(func); }
template<typename T, typename... Args> T* tmNew(Args&&... args) { return RedoOpt::tmNew<T>(args...); }
template<typename T> void tmDelete(T* obj) { RedoOpt::tmDelete<T>(obj); }
template<typename T> static T* get_object(int idx) { return RedoOpt::get_object<T>(idx); }
template<typename T> static void put_object(int idx, T* obj) { RedoOpt::put_object<T>(idx, obj); }
...
template<typename T> inline void persist<T>::pstore(T newVal) {
        ...
        if (offset != 0 && ADDR_IS_IN_MAIN(valaddr)) {
            ...
            if(oldval != (uint64_t)newVal) {
            	sameAddr = !gCX.addAddrIfAbsent(valaddr, oldval, (uint64_t)newVal);
            	*reinterpret_cast<T*>( valaddr + offset ) = newVal;
            }
           if(!tlocal.copy && !sameAddr) gCX.addIfAbsent(valaddr);
        } else if (ADDR_IS_IN_REGION(valaddr)) {
                if((uint64_t)val != (uint64_t)newVal){
                	sameAddr = !gCX.addAddrIfAbsent(valaddr - offset, (uint64_t)val, (uint64_t)newVal);
                	val = newVal;
                }
                if(!tlocal.copy && !sameAddr) gCX.addIfAbsent(valaddr - offset);
        }else{
        	val = newVal;
        }
}
