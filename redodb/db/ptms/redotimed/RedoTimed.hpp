...
class RedoTimed {
    ...
    static const int MAX_THREADS = 33;
    static const int MAX_COMBINEDS = 64;
...
private:
    ...
    inline void apply_redolog(State* state, uint64_t redoSize, WriteSetEntry* entries, WriteSetNode* tail, int tid) noexcept {
    ...
    inline Combined* apply_redologs(Combined* newComb, uint64_t initCombSeq, SeqTidIdx lastAppliedTicket, SeqTidIdx ltail, int tid) noexcept {
        ...
        for(;i<=lastSeq;i++){
            ...
            if(redoSize > 0){
                ...
                apply_redolog(apply_state, redoSize, entries, apply_state->logTail, tid);
...
public:
    ...
    inline void dbLog(void* addr, void* cpyaddr, size_t size) noexcept {
        ...
        while(true){
            ...
            add_to_log((RedoTimed::State*)tlocal.st, _addr, oldval,newval);
    ...
    bool makeCopy(Combined* newComb, int tid){
    	int lCombIndex;
		if((lCombIndex = getCombined(tid)) == -1) return false;
...
template<typename T> inline void persist<T>::pstore(T newVal) {
        ...
        if (offset != 0 && ADDR_IS_IN_MAIN(valaddr)) {
        	...
        	sameCL = gCX.add_to_log((RedoTimed::State*)tlocal.st, valaddr, oldval, (uint64_t)newVal);
           ...
           if(!sameCL) DEFER_PWB(ADDR2CL(valaddr + offset));
            ...
            if (ADDR_IS_IN_REGION(valaddr)) {
            	sameCL = gCX.add_to_log((RedoTimed::State*)tlocal.st, valaddr - offset, (uint64_t)val, (uint64_t)newVal);
