...
*/
#include "../redotimed/RedoTimed.hpp"
//modification by Andreia
#define ONLY_MSPACES 1
// Temporary disable the ok_address() check because it messes up the pointer adjustment on main/back
#define INSECURE 1
...
#define cinuse(p)           ((p)->head.pload() & CINUSE_BIT)
#define pinuse(p)           ((p)->head.pload() & PINUSE_BIT)
#define flag4inuse(p)       ((p)->head.pload() & FLAG4_BIT)
#define is_inuse(p)         (((p)->head.pload() & INUSE_BITS) != PINUSE_BIT)
#define is_mmapped(p)       (((p)->head.pload() & INUSE_BITS) == 0)

#define chunksize(p)        ((p)->head.pload() & ~(FLAG_BITS))
...
#define smallbin_at(M, i)   ((sbinptr)CX_ADJUSTED_ADDR((uint8_t*)&( (M)->smallbins[(i)<<1] )))
