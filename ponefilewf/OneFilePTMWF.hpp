...
#ifndef PM_FILE_NAME
#define PM_FILE_NAME   "/dev/shm/ponefilewf_shared"
#endif
...
#define FUNC_FLUSH  0
#define FUNC_LAMBDA 2
#define FUNC_APPLY  3
#define FUNC_UPDATE 4
...
struct VarLocal {
    const char* funcName[5] {"flush","logging","lambdas","apply","updateTx"};
    nanoseconds funcTime[5] {0ns, 0ns, 0ns, 0ns, 0ns};
    ~VarLocal() {
        uint64_t tmp =0;
...
struct PMetadata {
    ...
    uint64_t                id {0};
    uint64_t                pad2 {0};
    // We need a constructor to reset the rootPtrs' sequence (they're tmtypes)
    PMetadata() {
        std::memset(&rootPtrs, 0, sizeof(rootPtrs[MAX_ROOT_POINTERS]));
    }
};
...
struct WriteSetEntry {
    void*          addr;  // Address of value+sequence to change
    uint64_t       val;   // Desired value to change to
    WriteSetEntry* next;  // Pointer to next node in the (intrusive) hash map
};
...
struct WriteSet {
    ...
    inline void flushModifications() {
        START_TIME();
        for (uint64_t i = 0; i < numStores; i++) {
            //if (!lookupCacheLine(log[i].addr, i)) PWB(log[i].addr); // TODO: enable this optimization...
            PWB(log[i].addr);
        }
        END_TIME(FUNC_FLUSH);
    }
...
extern thread_local OpDesc* tl_opdesc;
...
public:
    ...
    OneFileWF() {
        ...
        opDesc = new OpDesc[REGISTRY_MAX_THREADS];
        ...
        // TODO: move this call outside the constructor, but for now, leave it here like on Romulus
    ...
    void mapPersistentRegion(const char* filename, uint8_t* regionAddr, const uint64_t regionSize) {
        ...
        // mmap() memory range
        uint8_t* got_addr = (uint8_t *)mmap(regionAddr, regionSize, (PROT_READ | PROT_WRITE), MAP_SHARED_VALIDATE | PM_FLAGS, fd, 0);
        ...
        for (uint64_t i = 0; i < REGISTRY_MAX_THREADS; i++) opDesc[i].pWriteSet = &(pmd->plog[i]);
        curTx = &(pmd->curTx);
    ...
    void retireRetiresFromLog(OpDesc& myopd, const int tid) {
    ...
    inline bool commitTx(OpDesc& myopd, const int tid) {
    ...
    inline void innerUpdateTx(OpDesc& myopd, TransFunc* funcptr, const int tid) {
...
private:
    ...
    inline void helpApply(uint64_t lcurTx, const int tid) {
        uint64_t idx = trans2idx(lcurTx);
        uint64_t seq = trans2seq(lcurTx);
        OpDesc& opd = opDesc[idx];
        ...
        uint64_t newReq = seqidx2trans(seq+1,idx);
    ...
    inline bool transformAll(const uint64_t lcurrTx, const int tid) {
        ...
        for (unsigned i = 0; i < ThreadRegistry::getMaxThreads(); i++) {
            ...
            if (lcurrTx != curTx->load(std::memory_order_acquire)) {
                END_TIME(FUNC_LAMBDA);
                return false;
            }
    ...
    // This is not needed on x85, where the DCAS has atomicity writting to persistent memory.
    void recover() {
        uint64_t lcurTx = curTx->load(std::memory_order_acquire);
        opDesc[trans2idx(lcurTx)].pWriteSet->applyFromRecover();
        PSYNC();
    }
};
...
template<typename T> inline void tmtype<T>::pstore(T newVal) {
    if (tl_opdesc == nullptr) { // Looks like we're outside a transaction
        val.store((uint64_t)newVal, std::memory_order_relaxed);
    } else {
        gOFWF.writeSets[tl_tcico.tid].addToLog(this, (uint64_t)newVal);
    }
}
...
//
OneFileWF gOFWF {};
thread_local OpDesc* tl_opdesc {nullptr};
